...
@Transactional
@Log4j
public class LotoManiaHistoricFileReaderService {
	...
	private DozenDTO buildHistoricDTO(String[] values, LotteryType type) {
		...
		return DozenDTO.builder().type(type).concurse(concurse).concurseDate(dateOfConcurse)
				...
				.dozen15(dozens.get(14)).dozen16(dozens.get(15))
				.dozen17(dozens.get(16)).dozen18(dozens.get(17))
				.dozen19(dozens.get(18)).dozen20(dozens.get(19))
				.build();
	}
	...
	private List<Integer> createOrdenedListOfIntegers(String[] values) {
		List<Integer> dozens = asList(valueOf(values[2]),valueOf(values[3]),valueOf(values[4]),valueOf(values[5]),valueOf(values[6]),valueOf(values[7]),
				valueOf(values[8]),valueOf(values[9]),valueOf(values[10]),valueOf(values[11]),valueOf(values[12]),valueOf(values[13]),valueOf(values[14])
				,valueOf(values[15]), valueOf(values[16]), valueOf(values[17]), valueOf(values[18]), valueOf(values[19]), valueOf(values[20]),valueOf(values[21]));
